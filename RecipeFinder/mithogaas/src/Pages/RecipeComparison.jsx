         ...
         <div className='bg-white border-b-2 border-gray-200 py-6 mb-8'>
          <div className='max-w-7xl mx-auto px-4'>
           ...
           <div className='flex gap-2 border-b border-gray-200'>
             <button onClick={() => setActiveTab('overview')} className={`px-4 py-2 font-semibold font-share transition-colors ${ activeTab === 'overview' ? 'border-b-2 border-[#58e633] text-[#58e633]' : 'text-gray-600 hover:text-gray-900' }`}>Overview</button>
                                    ...
                                    <div className='p-6'>
                                        ...
                                        <div className='space-y-3 mb-4'>
                                            ...
                                            {cookingTime && (
                                                <div className='flex justify-between py-2 border-b border-gray-200'>
                                                    ...
                                                    <span className='font-semibold font-share'>
                                                        {cookingTime >= 60 
                                                            ? `${Math.floor(cookingTime / 60)}h ${cookingTime % 60}m`
                                                            : `${cookingTime}m`
                                                        }
                                                    </span>
                                                </div>
                                        ...
                                        {recipe.strTags && (
                                            <div className='flex flex-wrap gap-2 mb-4'>
                                                {recipe.strTags.split(',').map((tag, i) => (
                                                    <span key={i} className='px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded-full font-share'>
                                                        {tag.trim()}
                                                    </span>
                                                ))}
                                            </div>
                                        ...
                                        <div className='flex gap-2'>
                                            <Link
                                                ...
                                                className='flex-1 px-4 py-2 bg-[#a7f1a0] text-black text-center font-semibold rounded-lg hover:bg-[#58e633] transition-colors font-share text-sm'
                                            >
                                                View Full Recipe
                                            </Link>
                                        </div>
                    ...
                    <div>
                        ...
                        {commonIngredients.length > 0 && (
                            <div className='bg-green-50 border-2 border-green-200 rounded-lg p-6 mb-6'>
                                <h3 className='text-lg font-bold mb-3 font-share text-green-800'>
                                    🎯 Common Ingredients ({commonIngredients.length})
                                </h3>
                                <div className='flex flex-wrap gap-2'>
                                    {commonIngredients.map((ing, i) => (
                                        <span key={i} className='px-3 py-1 bg-green-200 text-green-900 rounded-full text-sm font-semibold font-share capitalize'>
                                            {ing}
                                        </span>
                                    ))}
                                </div>
                                        ...
                                        <ul className='space-y-2'>
                                            {ingredients.map((ing, i) => {
                                                ...
                                                return (
                                                    <li 
                                                        key={i} 
                                                        className={`flex items-start gap-2 ${isCommon ? 'bg-green-50 px-2 py-1 rounded' : ''}`}
                                                    >
                                                        <span className={`w-2 h-2 mt-1.5 rounded-full flex-shrink-0 ${isCommon ? 'bg-green-500' : 'bg-gray-400'}`}></span>
                                                        <span className='text-sm font-share'>
                                                            <span className='font-semibold'>{ing.measure}</span> {ing.name}
                                                        </span>
                                                    </li>
                                                );
                                            })}
                                    ...
                                    <ol className='space-y-4'>
                                        {steps.map((step, i) => (
                                            <li key={i} className='flex gap-3'>
                                                <span className='flex-shrink-0 w-8 h-8 bg-[#a7f1a0] rounded-full flex items-center justify-center font-bold text-sm'>
                                                    {i + 1}
                                                </span>
